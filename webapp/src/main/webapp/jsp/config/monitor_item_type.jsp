                        ...
                        <div class="portlet-body">
                            <div class="table-toolbar">
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="btn-group">
                                            <button id="table_monitorItemType_new" class="btn green">
                                                添加 <i class="fa fa-plus"></i>
                                            </button>
                                            <button id="table_monitorItemType_delete" class="btn red">
                                    ...
                                    <%--<div class="col-md-6">--%>
                                        <%--<div class="btn-group pull-right">--%>
                                            <%--<button id="table_monitorItemType_export" class="btn yellow">--%>
                            ...
                            <table id="table_monitorItemType" class="table table-striped table-bordered table-hover">
                                <thead>
                                <tr>
                                    <th class="table-checkbox"><input type="checkbox" class="group-checkable"
                                                                      data-set="#table_monitorItemType .checkboxes"/></th>
                                    <th>监控项类型名称</th>
                                    <th>备 注</th>
                                    <th>创建日期</th>
                                    <th>操 作</th>
                                ...
                                <tbody>
                                <c:forEach items="${monitorItemTypeList}" var="monitorItemType">
                                    <tr class="odd gradeX">
                                        <td><input type="checkbox" class="checkboxes" value="${monitorItemType.id}"/></td>
                                        <td>${monitorItemType.name}</td>
                                        <td>${monitorItemType.desc}</td>
                                        <td>${monitorItemType.createDate}</td>
                                        <td>
                                            <a class="edit text-blue" id="edit_${monitorItemType.id}" href="javascript:;"><i
                                                    class="fa fa-pencil text-blue"></i> 编辑 </a>
                                            |
                                            <%--<c:if test="${monitorItemType.status == 1}">--%>
                                                <%--<a class="disable text-yellow" id="disable_${monitorItemType.id}" href="javascript:;"><i--%>
                                                        <%--class="fa fa-stop text-yellow"></i> 禁用 </a>--%>
                                            <%--</c:if>--%>
                                            <%--<c:if test="${monitorItemType.status == 0}">--%>
                                                <%--<a class="enable text-green" id="enable_${monitorItemType.id}" href="javascript:;"><i--%>
                                                        <%--class="fa fa-play text-green"></i> 启用 </a>--%>
                                            <%--</c:if>--%>
                                            <%--|--%>
                                            <a class="delete text-red" id="delete_${monitorItemType.id}" href="javascript:;"><i
...
<script src="/resources/scripts/config/monitorItemType.js" type="text/javascript"></script>

<script>
    jQuery(document).ready(function () {
        MonitorItemTypeTable.init();
    });
</script>
