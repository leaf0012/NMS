                        ...
                        <div class="portlet-body">
                            <div class="table-toolbar">
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="btn-group">
                                            <button id="table_alarmGroup_new" class="btn green">
                                                添加 <i class="fa fa-plus"></i>
                                            </button>
                                            <button id="table_alarmGroup_delete" class="btn red">
                                    ...
                                    <%--<div class="col-md-6">--%>
                                        <%--<div class="btn-group pull-right">--%>
                                            <%--<button id="table_alarmGroup_export" class="btn yellow">--%>
                            ...
                            <table id="table_alarmGroup" class="table table-striped table-bordered table-hover">
                                <thead>
                                <tr>
                                    <th class="table-checkbox"><input type="checkbox" class="group-checkable"
                                                                      data-set="#table_alarmGroup .checkboxes"/></th>
                                    <th>告警组名称</th>
                                    <th>创建日期</th>
                                    <th>备 注</th>
                                    <th>操 作</th>
                                ...
                                <tbody>
                                <c:forEach items="${alarmGroupList}" var="alarmGroup">
                                    <tr class="odd gradeX">
                                        <td><input type="checkbox" class="checkboxes" value="${alarmGroup.id}"/></td>
                                        <td>${alarmGroup.name}</td>
                                        <td>${alarmGroup.createDate}</td>
                                        <td>${alarmGroup.desc}</td>
                                        <td>
                                            <a class="edit text-blue" id="edit_${alarmGroup.id}" href="javascript:;"><i
                                                    class="fa fa-pencil text-blue"></i> 编辑 </a>
                                            |
                                            <%--<c:if test="${alarmGroup.status == 1}">--%>
                                                <%--<a class="disable text-yellow" id="disable_${alarmGroup.id}" href="javascript:;"><i--%>
                                                        <%--class="fa fa-stop text-yellow"></i> 禁用 </a>--%>
                                            <%--</c:if>--%>
                                            <%--<c:if test="${alarmGroup.status == 0}">--%>
                                                <%--<a class="enable text-green" id="enable_${alarmGroup.id}" href="javascript:;"><i--%>
                                                        <%--class="fa fa-play text-green"></i> 启用 </a>--%>
                                            <%--</c:if>--%>
                                            <%--|--%>
                                            <a class="delete text-red" id="delete_${alarmGroup.id}" href="javascript:;"><i
...
<script src="/resources/scripts/config/alarmGroup.js" type="text/javascript"></script>

<script>
    jQuery(document).ready(function () {
        AlarmGroupTable.init();
    });
</script>
