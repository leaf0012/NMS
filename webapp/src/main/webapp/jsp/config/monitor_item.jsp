...
<div class="wrapper">
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <h1>
                <i class="fa fa-dashboard"></i> Home > 系统配置 > 监控项管理 >
                <small>监控项列表</small>
            </h1>
        </section>
                        ...
                        <div class="portlet-body">
                            <div class="table-toolbar">
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="btn-group">
                                            <button id="table_monitorItem_new" class="btn green">
                                                添加 <i class="fa fa-plus"></i>
                                            </button>
                                            <button id="table_monitorItem_delete" class="btn red">
                                    ...
                                    <%--<button id="table_monitorItem_export" class="btn yellow">--%>
                            ...
                            <table id="table_monitorItem" class="table table-striped table-bordered table-hover">
                                <thead>
                                <tr>
                                    <th class="table-checkbox"><input type="checkbox" class="group-checkable"
                                                                      data-set="#table_monitorItem .checkboxes"/></th>
                                    <th>监控项编码</th>
                                    <th>监控项名称</th>
                                    <th>监控项类型</th>
                                    <th>创建日期</th>
                                ...
                                <tbody>
                                <c:forEach items="${monitorItemList}" var="monitorItem">
                                    <tr class="odd gradeX">
                                        <td><input type="checkbox" class="checkboxes" value="${monitorItem.id}"/></td>
                                        <td>${monitorItem.code}</td>
                                        <td>${monitorItem.name}</td>
                                        <td>${monitorItem.monitorItemType.name}</td>
                                        <td>${monitorItem.createDate}</td>
                                        <td>${monitorItem.desc}</td>
                                        <td>
                                            <a class="edit text-blue" id="edit_${monitorItem.id}" href="javascript:;"><i
                                                    class="fa fa-pencil text-blue"></i> 编辑 </a>
                                            |
                                            <a class="listProps text-green" id="editProps_${monitorItem.id}"
                                               href="javascript:;"><i
                                                    class="fa fa-pencil text-green"></i> 查看属性 </a>
                                            |
                                            <%--暂时屏蔽--%>
                                            <%--<c:if test="${monitorItem.status == 1}">--%>
                                                <%--<a class="disable text-yellow" id="disable_${monitorItem.id}"--%>
                                                   <%--href="javascript:;"><i--%>
                                                        <%--class="fa fa-stop text-yellow"></i> 禁用 </a>--%>
                                            <%--</c:if>--%>
                                            <%--<c:if test="${monitorItem.status == 0}">--%>
                                                <%--<a class="enable text-green" id="enable_${monitorItem.id}"--%>
                                                   <%--href="javascript:;"><i--%>
                                                        <%--class="fa fa-play text-green"></i> 启用 </a>--%>
                                            <%--</c:if>--%>
                                            <%--|--%>
                                            <a class="delete text-red" id="delete_${monitorItem.id}"
                                               href="javascript:;"><i
...
<script src="/resources/scripts/config/monitorItem.js" type="text/javascript"></script>

<script>
    jQuery(document).ready(function () {
        MonitorItemTable.init();
    });
</script>
